...
void host_dc4_server_init( VCHI_INSTANCE_T initialise_instance, VCHI_CONNECTION_T ** connections, uint32_t num_connections );
void host_dc4_client_init( VCHI_INSTANCE_T initialise_instance, VCHI_CONNECTION_T ** connections, uint32_t num_connections );
...
void update_display_info( fourcc_t service_id,int errors, int client_or_server )
{
   ...
   if (now - last_info_time > UPDATE_INFO_TIME) {
      ...
      for (i=0; i<MAX_INFO; i++) {
         ...
         printf("%c%c%c%c:%d (%d) %d\n", FOURCC_TO_CHAR(info[i].fourcc), info[i].messages, info[i].errors, info[i].av_messages);
      }
      if (i < 8) {
         test_failed = 1;
         assert(0);
