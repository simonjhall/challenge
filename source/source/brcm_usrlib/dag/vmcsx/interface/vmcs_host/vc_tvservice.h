...
File     :  $Id: //software/customers/broadcom/mobcom/vc4/rel/interface/vmcs_host/vc_tvservice.h#3 $
Revision :  $Revision: #3 $
 ...
 */
VCHPRE_ void vc_vchi_tv_init(VCHI_INSTANCE_T initialise_instance, VCHI_CONNECTION_T **connections, uint32_t num_connections );
...
/**
 * Host applications should call <DFN>vc_tv_register_callaback</DNF> at 
 ...
 *
 * @param mode (HDMI_MODE_HDMI/HDMI_MODE_DVI)
 *
 ...
 * modes for a particular standard (CEA/DMT). Prefer resolution (group and mode)
 * is also returned. If there are more modes supported than the size of the array
 ...
 * 
 * @param length of array above
 *
 * @pointer to preferred group
 *
 * @pointer to prefer mode code
 *
